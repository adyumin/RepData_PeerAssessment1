...
```{r echo=TRUE}
wdata<-data.frame(steps=newdata$steps, date=newdata$date, interval=newdata$interval,weekend=if (as.POSIXlt(strptime(newdata$date, "%Y-%m-%d"))$wday == 0) factor("weekend", levels=c("weekend", "weekday")) else factor("weekday", levels=c("weekend", "weekday")))
